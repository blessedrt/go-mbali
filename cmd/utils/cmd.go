...
func StartNode(ctx *cli.Context, stack *node.Node, isConsole bool) {
	...
	go func() {
		...
		minFreeDiskSpace := 2 * mblconfig.Defaults.TrieDirtyCache // Default 2 * 256Mb
...
func monitorFreeDiskSpace(sigc chan os.Signal, path string, freeDiskSpaceCritical uint64) {
	for {
		...
		if freeSpace < freeDiskSpaceCritical {
			log.Error("Low disk space. Gracefully shutting down gombl to prevent database corruption.", "available", common.StorageSize(freeSpace))
			sigc <- syscall.SIGTERM
			break
		} else if freeSpace < 2*freeDiskSpaceCritical {
			log.Warn("Disk space is running low. gombl will shutdown if disk space runs below critical level.", "available", common.StorageSize(freeSpace), "critical_level", common.StorageSize(freeDiskSpaceCritical))
		}
...
func ImportPreimages(db mbldb.Database, fn string) error {
...
// It's a part of the deprecated functionality, should be removed in the future.
func ExportPreimages(db mbldb.Database, fn string) error {
...
type exportHeader struct {
	Magic    string // Always set to 'gombldbdump' for disambiguation
	Version  uint64
	Kind     string
	UnixTime uint64
}

const exportMagic = "gombldbdump"
const (
	OpBatchAdd = 0
	OpBatchDel = 1
)
...
func ImportLDBData(db mbldb.Database, f string, startIndex int64, interrupt chan struct{}) error {
