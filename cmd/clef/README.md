...
Clef can be used to sign transactions and data and is meant as a(n eventual) replacement for gombl's account management. This allows DApps to not depend on gombl's account management. When a DApp wants to sign data (or a transaction), it can send the content to Clef, which will then provide the user with context and asks for permission to sign the content. If the users grants the signing request, Clef will send the signature back to the DApp.

This setup allows a DApp to connect to a remote mbali node and send transactions that are locally signed. This can help in situations when a DApp is connected to an untrusted remote mbali node, because a local one is not available, not synchronised with the chain, or is a node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.mbali/keystore")
...
* gombl todos
    - The signer should pass the `Origin` header as call-info to the UI. As of right now, the way that info about the request is put togombler is a bit of a hack into the HTTP server. This could probably be greatly improved.
    - Relay: gombl should be started in `gombl --signer localhost:8550`.
    - Currently, the gombl APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This type is 20 `bytes`, and is incapable of carrying checksum information. The signer uses `common.MixedcaseAddress`, which retains the original input.
    - The gombl API should switch to use the same type, and relay `to`-account verbatim to the external API.
* [x] Storage
...
Clef listens to HTTP requests on `http.addr`:`http.port` (or to IPC on `ipcpath`), with the same JSON-RPC standard as gombl. The messages are expected to be [JSON-RPC 2.0 standard](https://www.jsonrpc.org/specification).
...
#### Arguments
  - content type [string]: type of signed data
     ...
     - `application/clique`: [clique](https://github.com/mbali/EIPs/issues/225) headers
...
#### Sign data
   Signs a chunk of structured data conformant to [EIP-712](https://github.com/mbali/EIPs/blob/master/EIPS/eip-712.md) and returns the calculated signature.
  ...
  "params": [
    {
      "accounts": [
        {
          "url": "keystore:///home/bazonk/.mbali/keystore/UTC--2017-11-20T14-44-54.089682944Z--123409812340981234098123409812deadbeef42",
          "address": "0x123409812340981234098123409812deadbeef42"
        },
        {
          "url": "keystore:///home/bazonk/.mbali/keystore/UTC--2017-11-23T21-59-03.199240693Z--cafebabedeadbeef34098123409812deadbeef42",
...
* A Graphical UI MUST show the blocky-identicon for mbali addresses.
...
| Clef UI| https://github.com/mbali/clef-ui| Golang/QT-based| :+1:| :+1:| :x:| :+1:| :+1:| :x: |  :+1: (approve tx only)|
