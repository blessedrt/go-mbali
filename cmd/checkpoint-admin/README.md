...
*For a more detailed introduction to checkpoint, please see the LES [spec](https://github.com/mbali/devp2p/blob/master/caps/les.md).*
...
```shell
go get github.com/mbali/go-mbali/cmd/checkpoint-admin
```
...
It is worth noting that checkpoint-admin only supports clef as a signer for transactions and plain text(checkpoint). For more clef usage, please see the clef [tutorial](https://gombl.mbali.org/docs/clef/tutorial) .
...
*It is worth noting that the connected gombl node can be a fullnode or a light client. If it is fullnode, you must enable the LES protocol. E.G. add `--light.serv 50` to the startup command line flags*.
...
* Start gombl with the modified configuration file
