...
import (
	...
	"github.com/mbali/go-mbali/accounts/external"
...
type mblstatsConfig struct {
	URL string `toml:",omitempty"`
}

type gomblConfig struct {
	mbl      mblconfig.Config
	Node     node.Config
	mblstats mblstatsConfig
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "mbl")
	cfg.WSModules = append(cfg.WSModules, "mbl")
...
func makeConfigNode(ctx *cli.Context) (*node.Node, gomblConfig) {
	...
	utils.SetmblConfig(ctx, stack, &cfg.mbl)
	if ctx.GlobalIsSet(utils.mblStatsURLFlag.Name) {
		cfg.mblstats.URL = ctx.GlobalString(utils.mblStatsURLFlag.Name)
	}
...
func makeFullNode(ctx *cli.Context) (*node.Node, mblapi.Backend) {
	stack, cfg := makeConfigNode(ctx)
	if ctx.GlobalIsSet(utils.OverrideArrowGlacierFlag.Name) {
		cfg.mbl.OverrideArrowGlacier = new(big.Int).SetUint64(ctx.GlobalUint64(utils.OverrideArrowGlacierFlag.Name))
	}
	if ctx.GlobalIsSet(utils.OverrideTerminalTotalDifficulty.Name) {
		cfg.mbl.OverrideTerminalTotalDifficulty = utils.GlobalBig(ctx, utils.OverrideTerminalTotalDifficulty.Name)
	}
	backend, mbl := utils.RegistermblService(stack, &cfg.mbl)
	// Warn users to migrate if they have a legacy freezer format.
	if mbl != nil && !ctx.GlobalIsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		firstIdx := uint64(0)
		// Hack to speed up check for mainnet because we know
		// the first non-empty block.
		ghash := rawdb.ReadCanonicalHash(mbl.ChainDb(), 0)
		if cfg.mbl.NetworkId == 1 && ghash == params.MainnetGenesisHash {
			firstIdx = 46147
		}
		isLegacy, _, err := dbHasLegacyReceipts(mbl.ChainDb(), firstIdx)
		if err != nil {
	...
	// Add the mbali Stats daemon if requested.
	if cfg.mblstats.URL != "" {
		utils.RegistermblStatsService(stack, backend, cfg.mblstats.URL)
	}
...
func deprecated(field string) bool {
	switch field {
	case "mblconfig.Config.EVMInterpreter":
		return true
	case "mblconfig.Config.EWASMInterpreter":
		return true
	default:
		return false
	}
