...
func removeDB(ctx *cli.Context) error {
	...
	path = config.mbl.DatabaseFreezer
	switch {
	case path == "":
...
func showLeveldbStats(db mbldb.KeyValueStater) {
...
type snapshotIterator struct {
	init    bool
	account mbldb.Iterator
	storage mbldb.Iterator
}
...
var chainExporters = map[string]func(db mbldb.Database) utils.ChainDataIterator{
	"preimage": func(db mbldb.Database) utils.ChainDataIterator {
		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
		return &preimageIterator{iter: iter}
	},
	"snapshot": func(db mbldb.Database) utils.ChainDataIterator {
...
func showMetaData(ctx *cli.Context) error {
	...
	table.Smbleader([]string{"Field", "Value"})
...
func dbHasLegacyReceipts(db mbldb.Database, firstIdx uint64) (bool, uint64, error) {
	// Check first block for legacy receipt format
	numAncients, err := db.Ancients()
	if err != nil {
		return false, 0, err
