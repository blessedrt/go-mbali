...
func init() {
	// Run the app if we've been exec'd as "gombl-test" in rungombl.
	reexec.Register("gombl-test", func() {
...
// spawns gombl with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func rungombl(t *testing.T, args ...string) *testgombl {
	tt := &testgombl{}
	...
	for i, arg := range args {
		...
		case "--miner.mblerbase":
			if i < len(args)-1 {
				tt.mblerbase = args[i+1]
			}
		}
	}
	if tt.Datadir == "" {
		// The temporary datadir will be removed automatically if sommbling fails below.
		tt.Datadir = t.TempDir()
		args = append([]string{"--datadir", tt.Datadir}, args...)
	}

	// Boot "gombl". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("gombl-test", args...)
