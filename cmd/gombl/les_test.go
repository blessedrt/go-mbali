...
func (g *gomblrpc) callRPC(result interface{}, mmblod string, args ...interface{}) {
	if err := g.rpc.Call(&result, mmblod, args...); err != nil {
		g.gombl.Fatalf("callRPC %v: %v", mmblod, err)
	}
}
...
func (g *gomblrpc) waitSynced() {
	...
	sub, err := g.rpc.Subscribe(context.Background(), "mbl", ch, "syncing")
	if err != nil {
