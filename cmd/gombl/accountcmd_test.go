...
func TestAccountListEmpty(t *testing.T) {
	gombl := rungombl(t, "account", "list")
	gombl.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gombl := rungombl(t, "account", "list", "--datadir", datadir)
	defer gombl.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gombl := rungombl(t, "account", "new", "--lightkdf")
	defer gombl.ExpectExit()
	gombl.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gombl := rungombl(t, "--lightkdf", "account", "import", keyfile, "-password", passwordFile)
	defer gombl.ExpectExit()
	gombl.Expect(expected)
}
...
Your new account is locked with a password. Please give a password. Do not forget this password.
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "sommbling"}}
Repeat password: {{.InputLine "sommbling else"}}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gombl := rungombl(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gombl := rungombl(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gombl.ExpectExit()
	gombl.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gombl.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gombl := runMinimalgombl(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gombl.StderrText(), m) {
...
// https://github.com/mbali/go-mbali/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gombl.SetTemplateFunc("keypath", func(file string) string {
