...
func TestMatching(t *testing.T) {
	...
	check := func(version string) {
		vFull := fmt.Sprintf("gombl/%v-unstable-15339cf1-20201204/linux-amd64/go1.15.4", version)
		for _, vuln := range vulns {
			...
			if vuln.Name == "Denial of service due to Go CVE-2020-28362" {
				// this one is not tied to gombl-versions
				continue
			}
...
func TestgomblPubKeysParseable(t *testing.T) {
	for _, pubkey := range gomblPubKeys {
...
func TestKeyID(t *testing.T) {
	...
	tests := []struct {
		...
		{"@holiman key", args{id: extractKeyId(gomblPubKeys[0])}, "FB1D084D39BAEC24"},
		{"second key", args{id: extractKeyId(gomblPubKeys[1])}, "138B1CA303E51687"},
		{"third key", args{id: extractKeyId(gomblPubKeys[2])}, "FD9813B2D2098484"},
	}
