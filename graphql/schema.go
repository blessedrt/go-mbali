...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte mbali address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an mbali account at a particular block.
    type Account {
    ...
    # Transaction is an mbali transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an mbali account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an mbali account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an mbali block by number or by hash. If neither is
