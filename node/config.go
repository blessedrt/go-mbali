...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gombl is "gombl". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "gombl", keep that.
	if name == "gombl" || name == "gombl-testnet" {
		name = "gombl"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gombl" instances.
var isOldgomblResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gombl 1.4 are used if they exist.
	if warn, isOld := isOldgomblResource[path]; isOld {
		oldpath := ""
		if c.name() == "gombl" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldgomblResourceWarning, "Using deprecated resource file %s, please move this file to the 'gombl' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-mbali-keystore")
		isEphemeral = true
	}
