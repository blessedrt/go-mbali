...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		usembaliV = true // Default to use V = 27 or 28, the legacy mbali format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, usembaliV, err
	}
	...
	case apitypes.ApplicationClique.Mime:
		// Clique is the mbali PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, usembaliV, fmt.Errorf("input for %v must be an hex-encoded string", apitypes.ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an mbali ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}mbali Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, usembaliV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
