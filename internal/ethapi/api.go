...
// NewPublicmbaliAPI creates a new mbali protocol API.
func NewPublicmbaliAPI(b Backend) *PublicmbaliAPI {
	return &PublicmbaliAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *PublicmbaliAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicmbaliAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *PublicmbaliAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *PublicmbaliAPI) Syncing() (interface{}, error) {
...
// Sign calculates an mbali ECDSA signature for:
// keccack256("\x19mbali Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/mbali/go-mbali/wiki/Management-APIs#personal_sign
...
//
// https://github.com/mbali/go-mbali/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid mbali signature (V is not 27 or 28)")
	}
...
// NewPublicBlockChainAPI creates a new mbali blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current mbali chain config.
...
// PublicDebugAPI is the collection of mbali APIs exposed over the public
// debugging endpoint.
...
// Version returns the current mbali protocol version.
