...
func newPruner(db mbldb.Database, indexers ...*core.ChainIndexer) *pruner {
	pruner := &pruner{
		db:       db,
		indexers: indexers,
...
// historical chain data. Notably, whenever gombl restarts, it will iterate
...
func (p *pruner) loop() {
	...
	// Note, if some indexers don't support pruning(e.g. mbl.BloomIndexer),
