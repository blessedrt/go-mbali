...
//
// If you want to use the standardized mbali RPC functionality, use mblclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg mbali.CallMsg) (*types.AccessList, uint64, string, error) {
	...
	if err := ec.c.CallContext(ctx, &result, "mbl_createAccessList", toCallArg(msg)); err != nil {
		return nil, 0, "", err
	}
...
func (ec *Client) GetProof(ctx context.Context, account common.Address, keys []string, blockNumber *big.Int) (*AccountResult, error) {
	...
	err := ec.c.CallContext(ctx, &res, "mbl_getProof", account, keys, toBlockNumArg(blockNumber))
...
// the message call.
// Please use mblclient.CallContract instead if you don't need the override functionality.
func (ec *Client) CallContract(ctx context.Context, msg mbali.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
	var hex hexutil.Bytes
	err := ec.c.CallContext(
		ctx, &hex, "mbl_call", toCallArg(msg),
...
// Smblead sets the current head of the local chain by block number.
// Note, this is a destructive action and may severely damage your chain.
// Use with extreme caution.
func (ec *Client) Smblead(ctx context.Context, number *big.Int) error {
	return ec.c.CallContext(ctx, nil, "debug_smblead", toBlockNumArg(number))
}
...
func (ec *Client) SubscribePendingTransactions(ctx context.Context, ch chan<- common.Hash) (*rpc.ClientSubscription, error) {
	return ec.c.mblSubscribe(ctx, ch, "newPendingTransactions")
}
