...
// Verify that Client implements the mbali interfaces.
var (
	_ = mbali.ChainReader(&Client{})
	_ = mbali.TransactionReader(&Client{})
	_ = mbali.ChainStateReader(&Client{})
	_ = mbali.ChainSyncReader(&Client{})
	_ = mbali.ContractCaller(&Client{})
	_ = mbali.GasEstimator(&Client{})
	_ = mbali.GasPricer(&Client{})
	_ = mbali.LogFilterer(&Client{})
	_ = mbali.PendingStateReader(&Client{})
	// _ = mbali.PendingStateEventer(&Client{})
	_ = mbali.PendingContractCaller(&Client{})
)
...
func TestmblClient(t *testing.T) {
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == mbali.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != mbali.NotFound {
		t.Fatal("error should be mbali.NotFound")
	}
}
...
func testChainID(t *testing.T, client *rpc.Client) {
	...
	if id == nil || id.Cmp(params.AllmblashProtocolChanges.ChainID) != 0 {
