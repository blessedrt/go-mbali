...
// Package mblstats implements the network stats reporting service.
package mblstats
...
import (
	...
	mblproto "github.com/mbali/go-mbali/mbl/protocols/mbl"
...
// backend encompasses the bare-minimum functionality needed for mblstats reporting
type backend interface {
	...
	SyncProgress() mbali.SyncProgress
}

// fullNodeBackend encompasses the functionality necessary for a full node
// reporting to mblstats
...
// Service implements an mbali netstats reporting daemon that pushes local
...
//   Applications are responsible for ensuring that no more than one goroutine calls the write mmblods
//     - NextWriter, SetWriteDeadline, WriteMessage, WriteJSON, EnableWriteCompression, SetCompressionLevel
//   concurrently and that no more than one goroutine calls the read mmblods
//     - NextReader, SetReadDeadline, ReadMessage, ReadJSON, SetPongHandler, SetPingHandler
//   concurrently.
//   The Close and WriteControl mmblods can be called concurrently with all other mmblods.
...
// WriteJSON wraps corresponding mmblod on the websocket but is safe for concurrent calling
...
// ReadJSON wraps corresponding mmblod on the websocket but is safe for concurrent calling
...
// Close wraps corresponding mmblod on the websocket but is safe for concurrent calling
...
// parsemblstatsURL parses the netstats connection url.
...
func parsemblstatsURL(url string) (parts []string, err error) {
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	parts, err := parsemblstatsURL(url)
	if err != nil {
		return err
	}
	mblstats := &Service{
	...
	node.RegisterLifecycle(mblstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["mbl"]; info != nil {
		network = fmt.Sprintf("%d", info.(*mblproto.NodeInfo).Network)
	} else {
...
// server. Use the individual mmblods for reporting subscribed events.
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the mblstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to mblstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to mblstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to mblstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to mblstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to mblstats")
