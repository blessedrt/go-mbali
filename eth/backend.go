...
// Package eth implements the mbali protocol.
package eth
...
func New(stack *node.Node, config *ethconfig.Config) (*mbali, error) {
	// Ensure configuration values are compatible and sane
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run eth.mbali in light sync mode, use les.Lightmbali")
	}
...
func (s *mbali) Etherbase() (eb common.Address, err error) {
	s.lock.RLock()
...
func (s *mbali) SetEtherbase(etherbase common.Address) {
	s.lock.Lock()
	s.etherbase = etherbase
	s.lock.Unlock()
...
func (s *mbali) ChainDb() ethdb.Database            { return s.chainDb }
