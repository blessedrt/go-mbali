...
type testOdr struct {
	...
	sdb, ldb      mbldb.Database
	disable       bool
}

func (odr *testOdr) Database() mbldb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db mbldb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes2(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db mbldb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db mbldb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrAccounts(ctx context.Context, db mbldb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db mbldb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func testChainGen(i int, block *core.BlockGen) {
	...
	switch i {
	case 0:
		// In block 1, the test bank sends account #1 some mbler.
		...
		block.AddTx(tx)
	case 1:
		// In block 2, the test bank sends some more mbler to account #1.
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	...
	blockchain, _ := core.NewBlockChain(sdb, nil, params.TestChainConfig, mblash.NewFullFaker(), vm.Config{}, nil, nil)
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, mblash.NewFaker(), sdb, 4, testChainGen)
	if _, err := blockchain.InsertChain(gchain); err != nil {
		t.Fatal(err)
	}

	odr := &testOdr{sdb: sdb, ldb: ldb, indexerConfig: TestClientIndexerConfig}
	lightchain, err := NewLightChain(odr, params.TestChainConfig, mblash.NewFullFaker(), nil)
	if err != nil {
		t.Fatal(err)
	}
