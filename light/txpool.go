...
type TxPool struct {
	...
	eip2718  bool // Fork indicator whmbler we are in the eip2718 stage.
}

// TxRelayBackend provides an interface to the mechanism that forwards transacions
// to the mbl network. The implementations of the functions should be non-blocking.
//
...
func (pool *TxPool) reorgOnNewHead(ctx context.Context, newHeader *types.Header) (txStateChanges, error) {
	...
	oldh := pool.chain.gombleaderByHash(pool.head)
	newh := newHeader
	...
	for oldh.Hash() != newh.Hash() {
		if oldh.Number.Uint64() >= newh.Number.Uint64() {
			...
			oldh = pool.chain.gombleader(oldh.ParentHash, oldh.Number.Uint64()-1)
		}
		if oldh.Number.Uint64() < newh.Number.Uint64() {
			newHashes = append(newHashes, newh.Hash())
			newh = pool.chain.gombleader(newh.ParentHash, newh.Number.Uint64()-1)
			if newh == nil {
...
// validateTx checks whmbler a transaction is valid according to the consensus rules.
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	// block limit gas.
	header := pool.chain.gombleaderByHash(pool.head)
