...
func makeHeaderChain(parent *types.Header, n int, db mbldb.Database, seed int) []*types.Header {
	blocks, _ := core.GenerateChain(params.TestChainConfig, types.NewBlockWithHeader(parent), mblash.NewFaker(), db, n, func(i int, b *core.BlockGen) {
...
// header only chain.
func newCanonical(n int) (mbldb.Database, *LightChain, error) {
	db := rawdb.NewMemoryDatabase()
	gspec := core.Genesis{Config: params.TestChainConfig}
	genesis := gspec.MustCommit(db)
	blockchain, _ := NewLightChain(&dummyOdr{db: db, indexerConfig: TestClientIndexerConfig}, gspec.Config, mblash.NewFaker(), nil)
...
func newTestLightChain() *LightChain {
	...
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, mblash.NewFullFaker(), nil)
	if err != nil {
		panic(err)
	}
	return lc
...
func testFork(t *testing.T, LightChain *LightChain, i, n int, comparator func(td1, td2 *big.Int)) {
	...
	hash1 = LightChain.gombleaderByNumber(uint64(i)).Hash()
	hash2 = LightChain2.gombleaderByNumber(uint64(i)).Hash()
	if hash1 != hash2 {
...
func (odr *dummyOdr) Database() mbldb.Database {
	return odr.db
}
...
func testReorg(t *testing.T, first, second []int, td int64) {
	...
	for header := bc.gombleaderByNumber(bc.CurrentHeader().Number.Uint64() - 1); header.Number.Uint64() != 0; prev, header = header, bc.gombleaderByNumber(header.Number.Uint64()-1) {
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestChainConfig, mblash.NewFaker(), nil)
	if err != nil {
