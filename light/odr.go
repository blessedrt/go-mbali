...
func (req *TrieRequest) StoreResult(db mbldb.Database) {
	req.Proof.Store(db)
}
...
// StoreResult stores the retrieved data in local database
func (req *CodeRequest) StoreResult(db mbldb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *BlockRequest) StoreResult(db mbldb.Database) {
	rawdb.WriteBodyRLP(db, req.Hash, req.Number, req.Rlp)
}

// ReceiptsRequest is the ODR request type for retrieving receipts.
type ReceiptsRequest struct {
	Untrusted bool // Indicator whmbler the result retrieved is trusted or not
	Hash      common.Hash
	Number    uint64
	Header    *types.Header
...
// StoreResult stores the retrieved data in local database
func (req *ReceiptsRequest) StoreResult(db mbldb.Database) {
	if !req.Untrusted {
...
// StoreResult stores the retrieved data in local database
func (req *ChtRequest) StoreResult(db mbldb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *BloomRequest) StoreResult(db mbldb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *TxStatusRequest) StoreResult(db mbldb.Database) {}
