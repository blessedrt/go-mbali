...
func ReadSkeletonSyncStatus(db mbldb.KeyValueReader) []byte {
	data, _ := db.Get(skeletonSyncStatusKey)
	return data
}

// WriteSkeletonSyncStatus stores the serialized sync status to save at shutdown.
func WriteSkeletonSyncStatus(db mbldb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSkeletonSyncStatus(db mbldb.KeyValueWriter) {
...
func ReadSkeletonHeader(db mbldb.KeyValueReader, number uint64) *types.Header {
...
func WriteSkeletonHeader(db mbldb.KeyValueWriter, header *types.Header) {
...
func DeleteSkeletonHeader(db mbldb.KeyValueWriter, number uint64) {
