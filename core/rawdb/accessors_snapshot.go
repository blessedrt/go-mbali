...
func ReadSnapshotDisabled(db mbldb.KeyValueReader) bool {
	disabled, _ := db.Has(snapshotDisabledKey)
	return disabled
}

// WriteSnapshotDisabled stores the snapshot pause flag.
func WriteSnapshotDisabled(db mbldb.KeyValueWriter) {
...
func DeleteSnapshotDisabled(db mbldb.KeyValueWriter) {
...
func ReadSnapshotRoot(db mbldb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db mbldb.KeyValueWriter, root common.Hash) {
...
// the persisted snapshot. Since snapshots are not immutable, this  mmblod can
// be used during updates, so a crash or failure will mark the entire snapshot
// invalid.
func DeleteSnapshotRoot(db mbldb.KeyValueWriter) {
...
func ReadAccountSnapshot(db mbldb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db mbldb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db mbldb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db mbldb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db mbldb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db mbldb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db mbldb.Iteratee, accountHash common.Hash) mbldb.Iterator {
...
func ReadSnapshotJournal(db mbldb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db mbldb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db mbldb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db mbldb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db mbldb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db mbldb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db mbldb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db mbldb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db mbldb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db mbldb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db mbldb.KeyValueWriter, status []byte) {
