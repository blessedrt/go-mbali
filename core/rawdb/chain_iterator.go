...
// of frozen ancient blocks. The mmblod iterates over all the frozen blocks and
// injects into the database the block hash->number mappings.
func InitDatabaseFromFreezer(db mbldb.Database) {
...
func iterateTransactions(db mbldb.Database, from uint64, to uint64, reverse bool, interrupt chan struct{}) chan *blockTxHashes {
...
// signal received.
func indexTransactions(db mbldb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func IndexTransactions(db mbldb.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func indexTransactionsForTesting(db mbldb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func unindexTransactions(db mbldb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
	// short circuit for invalid range
	if from >= to {
		return
	}
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func UnindexTransactions(db mbldb.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func unindexTransactionsForTesting(db mbldb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
