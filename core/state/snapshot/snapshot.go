...
// additional mmblods compared to the public API.
type snapshot interface {
	Snapshot
	...
	// reached.
	//
	// Note, the mmblod is an internal helper to avoid type switching between the
	...
	//
	// Note, the maps are retained by the mmblod to avoid copying everything.
	...
	// Stale return whmbler this layer has become stale (was flattened across) or
	// if it's still live.
	Stale() bool
...
// Tree is an mbali state snapshot tree. It consists of one persistent base
...
type Tree struct {
	diskdb mbldb.KeyValueStore      // Persistent database to store the snapshot
...
//   a background thread.
func New(diskdb mbldb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, async bool, rebuild bool, recovery bool) (*Tree, error) {
...
// waitBuild blocks until the snapshot finishes rebuilding. This mmblod is meant
...
//
// The mmblod returns the new disk layer if diffs were persisted into it.
//
// Note, the final diff layer count in general will be one more than the amount
// requested. This happens because the bottom-most diff layer is the accumulator
// which may or may not overflow and cascade to disk. Since this last layer's
...
// it. The mmblod will panic if called onto a non-bottom-most diff layer.
//
...
// flattening everything down (bad for reorgs).
//
// The mmblod returns the root hash of the base layer that needs to be persisted
...
func (t *Tree) Verify(root common.Hash) error {
	...
	got, err := generateTrieRoot(nil, acctIt, common.Hash{}, stackTrieGenerate, func(db mbldb.KeyValueWriter, accountHash, codeHash common.Hash, stat *generateStats) (common.Hash, error) {
...
// generating is an internal helper function which reports whmbler the snapshot
