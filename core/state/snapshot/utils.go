...
func CheckDanglingStorage(chaindb mbldb.KeyValueStore) error {
...
func checkDanglingDiskStorage(chaindb mbldb.KeyValueStore) error {
	var (
		lastReport = time.Now()
		start      = time.Now()
		lastKey    []byte
...
// snapshot difflayers.
func checkDanglingMemStorage(db mbldb.KeyValueStore) error {
	start := time.Now()
...
func CheckJournalAccount(db mbldb.KeyValueStore, hash common.Hash) error {
