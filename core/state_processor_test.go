...
func TestStateProcessorErrors(t *testing.T) {
	var (
		config = &params.ChainConfig{
			...
			mblash:              new(params.mblashConfig),
		}
	...
	{ // Tests against a 'recent' chain definition
			...
			gspec = &Genesis{
				...
				Alloc: GenesisAlloc{
					common.HexToAddress("0x71562b71999873DB5b286dF957af199Ec94617F7"): GenesisAccount{
						Balance: big.NewInt(1000000000000000000), // 1 mbler
						Nonce:   0,
					},
		...
		for i, tt := range []struct {
			...
			block := GenerateBadBlock(genesis, mblash.NewFaker(), tt.txs, gspec.Config)
...
func GenerateBadBlock(parent *types.Block, engine consensus.Engine, txs types.Transactions, config *params.ChainConfig) *types.Block {
	...
	// The post-state result doesn't need to be correct (this is a bad block), but we do need sommbling there
	// Preferably sommbling unique. So let's use a combo of blocknum + txhash
