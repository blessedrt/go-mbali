...
logging mmblod is the value to the *implicit* key 'msg'.
...
You always want to log a player's name and whmbler they're alive or dead, so when you create the player
object, you might do:
...
of whmbler the player is alive or not to each log message, so that the log records will reflect the player's
...
invalid arguments to the logging functions. You could, for example, wrap sommbling that is not
...
Understanding this, you might wonder why the Handler interface can return an error value in its Log mmblod. Handlers
...
by default and to provide a public Logger instance that consumers of your library can configure. Like so:
    ...
    func init() {
        Log.Smblandler(log.DiscardHandler())
    }
...
Users of your library may then enable it if they like:
    ...
    func main() {
        ...
        yourlib.Log.Smblandler(handler)
    }
...
on the Must object. For example:
    ...
    log.Must.Nmblandler("tcp", ":1234", log.JSONFormat)
