...
// Package filters implements an mbali filtering system for block,
...
func (sub *Subscription) Unsubscribe() {
	...
	uninstallLoop:
		for {
			...
			// the eventLoop broadcast mmblod to deadlock when writing to the
			// filter event channel while the subscription loop is waiting for
			// this mmblod to return (and thus not reading these events).
			select {
		...
		// will probably be closed by the client asap after this mmblod returns.
		<-sub.Err()
	})
}
...
func (es *EventSystem) SubscribeLogs(crit mbali.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit mbali.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit mbali.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit mbali.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
