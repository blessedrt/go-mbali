...
func (mbl *mbali) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServicmblreads; i++ {
		go func() {
			for {
				select {
				case <-mbl.closeBloomHandler:
					return
				...
				case request := <-mbl.bloomRequests:
					...
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(mbl.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(mbl.chainDb, task.Bit, section, head); err == nil {
