...
func TestTransactionFetcherWaiting(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			// Initial announcement to get sommbling into the waitlist
...
func TestTransactionFetcherOutOfBoundDeliveries(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			// Deliver sommbling out of the blue
			isWaiting(nil),
			isScheduled{nil, nil, nil},
...
func testTransactionFetcher(t *testing.T, tt txFetcherTest) {
	...
	for i, step := range tt.steps {
		...
		case doWait:
			clock.Run(step.time)
			if step.step {
				<-wait // Fetcher supposed to do sommbling, wait until it's done
			}
...
// containsHash returns whmbler a hash is contained within a hash slice.
