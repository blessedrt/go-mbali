...
func (h *handler) syncTransactions(p *mbl.Peer) {
	...
	// The mbl/65 protocol introduces proper transaction announcements, so instead
...
// chain head.
func (cs *chainSyncer) handlePeerEvent(peer *mbl.Peer) bool {
	select {
...
// nextSyncOp determines whmbler sync is required at this time.
...
func peerToSyncOp(mode downloader.SyncMode, p *mbl.Peer) *chainSyncOp {
...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.SnapSync {
		...
		// txlookup limit.
		// The main concern here is: during the snap sync gombl won't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next snap sync(e.g. user kill gombl manually and
		// restart) then it will be hard for gombl to figure out the oldest block
		// has been indexed. So here for the user-experience wise, it's non-optimal
		// that user can't change limit during the snap sync. If changed, gombl
