...
// testmblHandler is a mock event handler to listen for inbound network requests
// on the `mbl` protocol and convert them into a more easily testable form.
type testmblHandler struct {
	blockBroadcasts event.Feed
	txAnnounces     event.Feed
	txBroadcasts    event.Feed
}

func (h *testmblHandler) Chain() *core.BlockChain              { panic("no backing chain") }
func (h *testmblHandler) TxPool() mbl.TxPool                   { panic("no backing tx pool") }
func (h *testmblHandler) AcceptTxs() bool                      { return true }
func (h *testmblHandler) RunPeer(*mbl.Peer, mbl.Handler) error { panic("not used in tests") }
func (h *testmblHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testmblHandler) Handle(peer *mbl.Peer, packet mbl.Packet) error {
	...
	default:
		panic(fmt.Sprintf("unexpected mbl packet type in tests: %T", packet))
	}
}

// Tests that peers are correctly accepted (or rejected) based on the advertised
// fork IDs in the protocol handshake.
func TestForkIDSplit66(t *testing.T) { testForkIDSplit(t, mbl.mbl66) }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	var (
		...
		mblNoFork, _ = newHandler(&handlerConfig{
			Database:   dbNoFork,
			Chain:      chainNoFork,
		...
		mblProFork, _ = newHandler(&handlerConfig{
			Database:   dbProFork,
			Chain:      chainProFork,
			TxPool:     newTestTxPool(),
			Merger:     consensus.NewMerger(rawdb.NewMemoryDatabase()),
	...
	mblNoFork.Start(1000)
	mblProFork.Start(1000)

	// Clean up everything after ourselves
	defer chainNoFork.Stop()
	defer chainProFork.Stop()

	defer mblNoFork.Stop()
	defer mblProFork.Stop()
	...
	peerNoFork := mbl.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pNoFork), p2pNoFork, nil)
	peerProFork := mbl.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pProFork), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- mblNoFork.runmblPeer(peerProFork, func(peer *mbl.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- mblProFork.runmblPeer(peerNoFork, func(peer *mbl.Peer) error { return nil })
	}(errc)
	...
	peerNoFork = mbl.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork = mbl.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()
...
func TestRecvTransactions66(t *testing.T) { testRecvTransactions(t, mbl.mbl66) }
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := mbl.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, handler.txpool)
	sink := mbl.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runmblPeer(sink, func(peer *mbl.Peer) error {
		return mbl.Handle((*mblHandler)(handler.handler), peer)
	})
...
func TestSendTransactions66(t *testing.T) { testSendTransactions(t, mbl.mbl66) }
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runmblPeer(src, func(peer *mbl.Peer) error {
		return mbl.Handle((*mblHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	for len(seen) < len(insert) {
		...
		case 66:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post mbl/66")
			}
...
func TestTransactionPropagation66(t *testing.T) { testTransactionPropagation(t, mbl.mbl66) }
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := mbl.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{byte(i + 1)}, "", nil, sourcePipe), sourcePipe, source.txpool)
		sinkPeer := mbl.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runmblPeer(sourcePeer, func(peer *mbl.Peer) error {
			return mbl.Handle((*mblHandler)(source.handler), peer)
		})
		go sink.handler.runmblPeer(sinkPeer, func(peer *mbl.Peer) error {
			return mbl.Handle((*mblHandler)(sink.handler), peer)
		})
	}
...
// Tests that post mbl protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := mbl.NewPeer(mbl.mbl66, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pLocal), p2pLocal, handler.txpool)
	remote := mbl.NewPeer(mbl.mbl66, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pRemote), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
	...
	go func() {
		...
		handler.handler.runmblPeer(local, func(peer *mbl.Peer) error {
			return mbl.Handle((*mblHandler)(handler.handler), peer)
		})
	}()
	...
	if checkpoint {
		...
		request := new(mbl.GetBlockHeadersPacket66)
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	sinks := make([]*testmblHandler, peers)
	for i := 0; i < len(sinks); i++ {
		sinks[i] = new(testmblHandler)
	}
	// Interconnect all the sink handlers with the source handler
	var (
	...
	for i, sink := range sinks {
		...
		sourcePeer := mbl.NewPeer(mbl.mbl66, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := mbl.NewPeer(mbl.mbl66, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func TestBroadcastMalformedBlock66(t *testing.T) { testBroadcastMalformedBlock(t, mbl.mbl66) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := mbl.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, source.txpool)
	sink := mbl.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runmblPeer(src, func(peer *mbl.Peer) error {
		return mbl.Handle((*mblHandler)(source.handler), peer)
	})
