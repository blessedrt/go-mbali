...
func (mbl *mbali) StateAtBlock(block *types.Block, reexec uint64, base *state.StateDB, checkLive bool, preferDisk bool) (statedb *state.StateDB, err error) {
	var (
		current  *types.Block
	...
	if checkLive {
		statedb, err = mbl.blockchain.StateAt(block.Root())
		if err == nil {
			return statedb, nil
		}
	}
	if base != nil {
		if preferDisk {
			...
			database = state.NewDatabaseWithConfig(mbl.chainDb, &trie.Config{Cache: 16})
		...
		current = mbl.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	} else {
		...
		for i := uint64(0); i < reexec; i++ {
			...
			parent := mbl.blockchain.GetBlock(current.ParentHash(), current.NumberU64()-1)
			if parent == nil {
	...
	for current.NumberU64() < origin {
		...
		if current = mbl.blockchain.GetBlockByNumber(next); current == nil {
			return nil, fmt.Errorf("block #%d not found", next)
		}
		_, _, _, err := mbl.blockchain.Processor().Process(current, statedb, vm.Config{})
		if err != nil {
			return nil, fmt.Errorf("processing block %d failed: %v", current.NumberU64(), err)
		}
		// Finalize the state so any modifications are written to the trie
		root, err := statedb.Commit(mbl.blockchain.Config().IsEIP158(current.Number()))
		if err != nil {
...
func (mbl *mbali) stateAtTransaction(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, error) {
	// Short circuit if it's genesis block.
	if block.NumberU64() == 0 {
		return nil, vm.BlockContext{}, nil, errors.New("no transaction in genesis")
	}
	// Create the parent state database
	parent := mbl.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, vm.BlockContext{}, nil, fmt.Errorf("parent %#x not found", block.ParentHash())
	}
	// Lookup the statedb of parent block from the live database,
	// otherwise regenerate it on the flight.
	statedb, err := mbl.StateAtBlock(parent, reexec, nil, true, false)
	if err != nil {
	...
	signer := types.MakeSigner(mbl.blockchain.Config(), block.Number())
	for idx, tx := range block.Transactions() {
		// Assemble the transaction call message and return if the requested offset
		msg, _ := tx.AsMessage(signer, block.BaseFee())
		txContext := core.NewEVMTxContext(msg)
		context := core.NewEVMBlockContext(block.Header(), mbl.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, txContext, statedb, mbl.blockchain.Config(), vm.Config{})
