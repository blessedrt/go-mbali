...
func TestTraceCall(t *testing.T) {
	...
	genesis := &core.Genesis{Alloc: core.GenesisAlloc{
		accounts[0].addr: {Balance: big.NewInt(params.mbler)},
		accounts[1].addr: {Balance: big.NewInt(params.mbler)},
		accounts[2].addr: {Balance: big.NewInt(params.mbler)},
	}}
	genBlocks := 10
	...
	var testSuite = []struct {
		...
		},
		{
			...
			config: &TraceCallConfig{
				BlockOverrides: &mblapi.BlockOverrides{Number: (*hexutil.Big)(big.NewInt(0x1337))},
			},
			expectErr: nil,
...
func TestTracingWithOverrides(t *testing.T) {
	...
	var testSuite = []struct {
		...
		// Call which can only succeed if state is state overridden
		{
			...
			config: &TraceCallConfig{
				StateOverrides: &mblapi.StateOverride{
					randomAccounts[0].addr: mblapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.mbler)))},
				},
			},
		...
		//  }
		{
			...
			config: &TraceCallConfig{
				//Tracer: &tracer,
				StateOverrides: &mblapi.StateOverride{
					randomAccounts[2].addr: mblapi.OverrideAccount{
