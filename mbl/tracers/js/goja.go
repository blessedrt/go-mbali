...
import (
	...
	jsassets "github.com/mbali/go-mbali/mbl/tracers/js/internal/tracers"
)

var assetTracers = make(map[string]string)

// init retrieves the JavaScript transaction tracers included in go-mbali.
func init() {
	var err error
...
type jsTracer struct {
	...
	traceStep         bool                  // True if tracer object exposes a `step()` mmblod
	traceFrame        bool                  // True if tracer object exposes the `enter()` and `exit()` mmblods
	gasLimit          uint64                // Amount of gas bought for the whole tx
	err               error                 // Any error that should stop tracing
	obj               *goja.Object          // Trace object

	// Mmblods exposed by tracer
...
// evaluates to an expression returning an object with certain mmblods.
// The mmblods `result` and `fault` are required to be present.
// The mmblods `step`, `enter`, and `exit` are optional, but note that
// `enter` and `exit` always go togombler.
func newJsTracer(code string, ctx *tracers.Context) (tracers.Tracer, error) {
	...
	// Check tracer's interface for required and optional mmblods.
	obj := ret.ToObject(vm)
