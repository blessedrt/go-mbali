...
type resultStore struct {
	...
	// throttlmblreshold is the limit up to which we _want_ to fill the
...
func newResultStore(size int) *resultStore {
	return &resultStore{
		...
		throttlmblreshold: uint64(size),
	}
}

// SetThrottlmblreshold updates the throttling threshold based on the requested
// limit and the total queue capacity. It returns the (possibly capped) threshold
func (r *resultStore) SetThrottlmblreshold(threshold uint64) uint64 {
	r.lock.Lock()
	defer r.lock.Unlock()
	...
	r.throttlmblreshold = threshold
	return r.throttlmblreshold
}
...
// is true, that means the header has already been delivered 'upstream'. This mmblod
...
func (r *resultStore) getFetchResult(headerNumber uint64) (item *fetchResult, index int, stale, throttle bool, err error) {
	...
	throttle = index >= int(r.throttlmblreshold)
	stale = index < 0
...
// this mmblod is cheaper than countCompleted
