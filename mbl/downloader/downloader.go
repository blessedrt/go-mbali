...
type Downloader struct {
	...
	// Skeleton sync
	skeleton *skeleton // Header skeleton to backfill the chain with (mbl2 mode)
...
func New(checkpoint uint64, stateDb mbldb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn, success func()) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() mbali.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td, ttd *big.Int, beaconMode bool) (err error) {
	...
	if mode == SnapSync {
		...
		if origin+1 < frozen {
			if err := d.lightchain.Smblead(origin); err != nil {
				return err
			}
		}
	}
...
// and going backwards towards genesis. This mmblod assumes that the caller already has
