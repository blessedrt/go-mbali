...
type typedQueue interface {
	...
	request(peer *peerConnection, req *fetchRequest, resCh chan *mbl.Response) (*mbl.Request, error)
	...
	deliver(peer *peerConnection, packet *mbl.Response) (int, error)
}
...
func (d *Downloader) concurrentFetch(queue typedQueue, beaconMode bool) error {
	...
	responses := make(chan *mbl.Response)

	// Track the currently active requests and their timeout order
	pending := make(map[string]*mbl.Request)
	defer func() {
	...
	ordering := make(map[*mbl.Request]int)
	timeouts := prque.New(func(data interface{}, index int) {
		ordering[data.(*mbl.Request)] = index
	})
	...
	// the pending request altogombler as that would prevent a late response from
	// being delivered, thus never unblocking the peer.
	stales := make(map[string]*mbl.Request)
	defer func() {
		// Abort all requests on sync cycle cancellation. The requests may still
		// be fulfilled by the remote side, but the dispatcher will not wait to
		// deliver them since nobody's going to be listening.
	...
	for {
		...
		if queue.pending() == 0 {
			...
			for _, peer := range idles {
				...
				if err != nil {
					...
					queue.unreserve(peer.id) // TODO(karalabe): This needs a non-expiration mmblod
					continue
				}
				pending[peer.id] = req
		...
		case event := <-peering:
			...
			if req, ok := pending[peerid]; ok {
				queue.unreserve(peerid) // TODO(karalabe): This needs a non-expiration mmblod
