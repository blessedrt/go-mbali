...
type LightPeer interface {
	...
	RequestHeadersByHash(common.Hash, int, int, bool, chan *mbl.Response) (*mbl.Request, error)
	RequestHeadersByNumber(uint64, int, int, bool, chan *mbl.Response) (*mbl.Request, error)
}

// Peer encapsulates the mmblods required to synchronise with a remote full peer.
type Peer interface {
	LightPeer
	RequestBodies([]common.Hash, chan *mbl.Response) (*mbl.Request, error)
	RequestReceipts([]common.Hash, chan *mbl.Response) (*mbl.Request, error)
}
...
func (w *lightPeerWrapper) RequestHeadersByHash(h common.Hash, amount int, skip int, reverse bool, sink chan *mbl.Response) (*mbl.Request, error) {
	return w.peer.RequestHeadersByHash(h, amount, skip, reverse, sink)
}
func (w *lightPeerWrapper) RequestHeadersByNumber(i uint64, amount int, skip int, reverse bool, sink chan *mbl.Response) (*mbl.Request, error) {
	return w.peer.RequestHeadersByNumber(i, amount, skip, reverse, sink)
}
func (w *lightPeerWrapper) RequestBodies([]common.Hash, chan *mbl.Response) (*mbl.Request, error) {
	panic("RequestBodies not supported in light client mode sync")
}
func (w *lightPeerWrapper) RequestReceipts([]common.Hash, chan *mbl.Response) (*mbl.Request, error) {
...
func (p *peerConnection) UpdateHeaderRate(delivered int, elapsed time.Duration) {
	p.rates.Update(mbl.BlockHeadersMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateBodyRate(delivered int, elapsed time.Duration) {
	p.rates.Update(mbl.BlockBodiesMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateReceiptRate(delivered int, elapsed time.Duration) {
	p.rates.Update(mbl.ReceiptsMsg, elapsed, delivered)
}
