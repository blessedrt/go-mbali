...
// the process. This is usually the case when gombl is shutting down and some events
...
// data on disk when two subchains are joined togombler.
...
// The update might request whmbler to forcefully change the target, or only try to
// extend it and fail if it's not possible.
type headUpdate struct {
	header *types.Header // Header to update the sync target to
	force  bool          // Whmbler to force the update or only extend if possible
...
type backfiller interface {
	...
	// on initial sartup.
	//
	// The mmblod should return the last block header that has been successfully
...
//
// Opposed to the original mbali block synchronization which is trustless (and
...
// not from within (vs. legacy mbali sync).
//
...
type skeleton struct {
	db     mbldb.Database // Database backing the skeleton
...
func newSkeleton(db mbldb.Database, peers *peerSet, drop peerDropFn, filler backfiller) *skeleton {
	sk := &skeleton{
		db:         db,
		filler:     filler,
		peers:      peers,
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but gombl is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until gombl requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
...
// ancestor.
//
// This mmblod does not block, rather it just waits until the syncer receives the
...
func (s *skeleton) sync(head *types.Header) (*types.Header, error) {
	...
	defer func() {
		if filled := s.filler.suspend(); filled != nil {
			// If sommbling was filled, try to delete stale sync helpers. If
	...
	// ephemeral so a data race doesn't accidentally deliver sommbling stale on
	...
	// Whmbler sync completed or not, disregard any future packets
	defer func() {
	...
	for {
		// Sommbling happened, try to assign new tasks to any idle peers
		if !linked {
			s.assignTasks(responses, requestFails, cancel)
		}
		// Wait for sommbling to happen
		select {
		case event := <-peering:
...
// is nil, the mmblod will return and continue from the previous head.
	...
	if status := rawdb.ReadSkeletonSyncStatus(s.db); len(status) > 0 {
		...
		if err := json.Unmarshal(status, s.progress); err != nil {
			...
			for len(s.progress.Subchains) > 0 {
				// If the last chain is above the new head, delete altogombler
...
func (s *skeleton) saveSyncStatus(db mbldb.KeyValueWriter) {
...
func (s *skeleton) assignTasks(success chan *headerResponse, fail chan *headerRequest, cancel chan struct{}) {
	...
	for _, peer := range s.idles {
		...
		idlers.caps = append(idlers.caps, s.peers.rates.Capacity(peer.id, mbl.BlockHeadersMsg, targetTTL))
	}
...
// arrives or a timeouts / cancellation is triggered. The mmblod should be run
...
func (s *skeleton) executeTask(peer *peerConnection, req *headerRequest) {
	...
	case <-timeoutTimer.C:
		...
		s.peers.rates.Update(peer.id, mbl.BlockHeadersMsg, 0, 0)
		s.scheduleRevertRequest(req)
	...
	case res := <-resCh:
		...
		headers := *res.Res.(*mbl.BlockHeadersPacket)

		headerReqTimer.Update(time.Since(start))
		s.peers.rates.Update(peer.id, mbl.BlockHeadersMsg, res.Time, len(headers))
...
func (s *skeleton) processResponse(res *headerResponse) (linked bool, merged bool) {
	...
	// Whmbler the response is valid, we can mark the peer as idle and notify
...
func (s *skeleton) cleanStales(filled *types.Header) error {
	...
	// If the filled header is below the linked subchain, sommbling's
...
// This mmblod is used by the backfiller, whose life cycle is controlled by the
// skeleton syncer.
//
// Note, the mmblod will not use the internal state of the skeleton, but will
...
// of this mmblod. Any head reorg will first tear down the backfiller and only
...
// Header retrieves a specific header tracked by the skeleton syncer. This mmblod
// is meant to be used by the backfiller, whose life cycle is controlled by the
// skeleton syncer.
//
// Note, outside the permitted runtimes, this mmblod might return nil results and
