...
// Backend defines the data retrieval mmblods to serve remote requests and the
// callback mmblods to invoke on remote deliveries.
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `mbl` protocol. The handler
...
// TxPool defines the mmblods needed by the protocol handler to serve transactions.
type TxPool interface {
	// Get retrieves the transaction from the local txpool with the given hash.
	Get(hash common.Hash) *types.Transaction
}

// MakeProtocols constructs the P2P protocol definitions for `mbl`.
...
// NodeInfo represents a short summary of the `mbl` sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // mbali network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
...
// nodeInfo retrieves some `mbl` protocol metadata about the running host node.
...
// Handle is invoked whenever an `mbl` connection is made that successfully passes
// the protocol handshake. This mmblod will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `mbl`", "err", err)
			return err
		}
	}
}
...
var mbl66 = map[uint64]msgHandler{
...
func handleMessage(backend Backend, peer *Peer) error {
	...
	var handlers = mbl66
	//if peer.Version() >= mbl67 { // Left in as a sample when new protocol is added
	//	handlers = mbl67
	//}
