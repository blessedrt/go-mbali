...
// enrEntry is the ENR entry which advertises `mbl` protocol on the discovery.
...
// StartENRUpdater starts the `mbl` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `mbl` ENR entry based on the current state of the chain.
