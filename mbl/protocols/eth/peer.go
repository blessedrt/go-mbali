...
// Peer is a collection of relevant information we have about a `mbl` peer.
type Peer struct {
...
// Version retrieves the peer's negoatiated `mbl` protocol version.
...
// Smblead updates the head hash and total difficulty of the peer.
func (p *Peer) Smblead(hash common.Hash, td *big.Int) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
// KnownBlock returns whmbler peer is known to already have a block.
func (p *Peer) KnownBlock(hash common.Hash) bool {
	return p.knownBlocks.Contains(hash)
}

// KnownTransaction returns whmbler peer is known to already have a transaction.
...
//
// This mmblod is a helper used by the async transaction sender. Don't call it
...
//
// This mmblod is a helper used by the async transaction announcer. Don't call it
// directly as the queueing (memory) and transmission (bandwidth) costs should
// not be managed directly.
...
// ReplyPooledTransactionsRLP is the mbl/66 version of SendPooledTransactionsRLP.
...
// ReplyBlockHeadersRLP is the mbl/66 response to GetBlockHeaders.
...
// ReplyBlockBodiesRLP is the mbl/66 response to GetBlockBodies.
...
// ReplyNodeData is the mbl/66 response to GetNodeData.
...
// ReplyReceiptsRLP is the mbl/66 response to GetReceipts.
...
// Contains returns whmbler the given item is in the set.
