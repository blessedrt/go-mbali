...
var (
	...
	// testAddr is the mbali address of the tester account.
...
func Testmbl2AssembleBlock(t *testing.T) {
	genesis, blocks := generatePreMergeChain(10)
	n, mblservice := startmblService(t, genesis, blocks)
	defer n.Close()

	api := NewConsensusAPI(mblservice)
	signer := types.NewEIP155Signer(mblservice.BlockChain().Config().ChainID)
...
func Testmbl2AssembleBlockWithAnotherBlocksTxs(t *testing.T) {
	genesis, blocks := generatePreMergeChain(10)
	n, mblservice := startmblService(t, genesis, blocks[:9])
	defer n.Close()

	api := NewConsensusAPI(mblservice)

	// Put the 10th block's tx in the pool and produce a new block
	api.mbl.TxPool().AddRemotesSync(blocks[9].Transactions())
	blockParams := beacon.PayloadAttributesV1{
...
func TestSmbleadBeforeTotalDifficulty(t *testing.T) {
	genesis, blocks := generatePreMergeChain(10)
	n, mblservice := startmblService(t, genesis, blocks)
	defer n.Close()
...
func Testmbl2PrepareAndGetPayload(t *testing.T) {
	genesis, blocks := generatePreMergeChain(10)
	...
	// Put the 10th block's tx in the pool and produce a new block
	mblservice.TxPool().AddLocals(blocks[9].Transactions())
	blockParams := beacon.PayloadAttributesV1{
		Timestamp: blocks[8].Time() + 5,
	}
	fcState := beacon.ForkchoiceStateV1{
...
func TestInvalidPayloadTimestamp(t *testing.T) {
	...
	n, mblservice := startmblService(t, genesis, preMergeBlocks)
	defer n.Close()
	var (
		api    = NewConsensusAPI(mblservice)
		parent = mblservice.BlockChain().CurrentBlock()
	)
	tests := []struct {
		time      uint64
		shouldErr bool
...
func Testmbl2NewBlock(t *testing.T) {
	genesis, preMergeBlocks := generatePreMergeChain(10)
	n, mblservice := startmblService(t, genesis, preMergeBlocks)
	defer n.Close()
	...
	mblservice.BlockChain().SubscribeLogsEvent(newLogCh)
	mblservice.BlockChain().SubscribeRemovedLogsEvent(rmLogsCh)

	for i := 0; i < 10; i++ {
		statedb, _ := mblservice.BlockChain().StateAt(parent.Root())
		nonce := statedb.GetNonce(testAddr)
		tx, _ := types.SignTx(types.NewContractCreation(nonce, new(big.Int), 1000000, big.NewInt(2*params.InitialBaseFee), logCode), types.LatestSigner(mblservice.BlockChain().Config()), testKey)
		mblservice.TxPool().AddLocal(tx)
		...
		if mblservice.BlockChain().CurrentBlock().NumberU64() != block.NumberU64()-1 {
		...
		if mblservice.BlockChain().CurrentBlock().NumberU64() != block.NumberU64() {
	...
	var (
		head = mblservice.BlockChain().CurrentBlock().NumberU64()
	)
	parent = preMergeBlocks[len(preMergeBlocks)-1]
	for i := 0; i < 10; i++ {
		execData, err := assembleBlock(api, parent.Hash(), &beacon.PayloadAttributesV1{
		...
		if mblservice.BlockChain().CurrentBlock().NumberU64() != head {
			t.Fatalf("Chain head shouldn't be updated")
		}
...
func Testmbl2DeepReorg(t *testing.T) {
	// TODO (MariusVanDerWijden) Testmbl2DeepReorg is currently broken, because it tries to reorg
	...
	/*
		...
		var (
			api    = NewConsensusAPI(mblservice, nil)
			parent = preMergeBlocks[len(preMergeBlocks)-core.TriesInMemory-1]
			head   = mblservice.BlockChain().CurrentBlock().NumberU64()
		)
		if mblservice.BlockChain().HasBlockAndState(parent.Hash(), parent.NumberU64()) {
		...
		for i := 0; i < 10; i++ {
			...
			block, err := ExecutableDataToBlock(mblservice.BlockChain().Config(), parent.Header(), *execData)
			if err != nil {
				t.Fatalf("Failed to convert executable data to block %v", err)
			}
			...
			if err := api.smblead(block.Hash()); err != nil {
...
// startmblService creates a full node instance for testing.
func startmblService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *mbl.mbali) {
	t.Helper()
	...
	mblcfg := &mblconfig.Config{Genesis: genesis, mblash: mblash.Config{PowMode: mblash.ModeFake}, SyncMode: downloader.SnapSync, TrieTimeout: time.Minute, TrieDirtyCache: 256, TrieCleanCache: 256}
	mblservice, err := mbl.New(n, mblcfg)
	if err != nil {
		t.Fatal("can't create mbl service:", err)
	}
	if err := n.Start(); err != nil {
		t.Fatal("can't start node:", err)
	}
	if _, err := mblservice.BlockChain().InsertChain(blocks); err != nil {
		n.Close()
		t.Fatal("can't import test blocks:", err)
	}
	time.Sleep(500 * time.Millisecond) // give txpool enough time to consume head event

	mblservice.Setmblerbase(testAddr)
	mblservice.SetSynced()
	return n, mblservice
}
...
func TestFullAPI(t *testing.T) {
	...
	setupBlocks(t, mblservice, 10, parent, callback)
}

func setupBlocks(t *testing.T, mblservice *mbl.mbali, n int, parent *types.Block, callback func(parent *types.Block)) {
	api := NewConsensusAPI(mblservice)
	for i := 0; i < n; i++ {
		callback(parent)
		...
		if mblservice.BlockChain().CurrentBlock().NumberU64() != payload.Number {
			t.Fatal("Chain head should be updated")
		}
		if mblservice.BlockChain().CurrentFinalizedBlock().NumberU64() != payload.Number-1 {
...
func assembleBlock(api *ConsensusAPI, parentHash common.Hash, params *beacon.PayloadAttributesV1) (*beacon.ExecutableDataV1, error) {
	block, err := api.mbl.Miner().GetSealingBlockSync(parentHash, params.Timestamp, params.SuggestedFeeRecipient, params.Random, false)
	if err != nil {
		return nil, err
	}
...
func TestEmptyBlocks(t *testing.T) {
	...
	commonAncestor := mblservice.BlockChain().CurrentBlock()
	api := NewConsensusAPI(mblservice)

	// Setup 10 blocks on the canonical chain
	setupBlocks(t, mblservice, 10, commonAncestor, func(parent *types.Block) {})
...
func TestTrickRemoteBlockCache(t *testing.T) {
	// Setup two nodes
	genesis, preMergeBlocks := generatePreMergeChain(10)
	nodeA, mblserviceA := startmblService(t, genesis, preMergeBlocks)
	nodeB, mblserviceB := startmblService(t, genesis, preMergeBlocks)
	defer nodeA.Close()
	defer nodeB.Close()
	...
	apiA := NewConsensusAPI(mblserviceA)
	apiB := NewConsensusAPI(mblserviceB)

	commonAncestor := mblserviceA.BlockChain().CurrentBlock()

	// Setup 10 blocks on the canonical chain
	setupBlocks(t, mblserviceA, 10, commonAncestor, func(parent *types.Block) {})
	commonAncestor = mblserviceA.BlockChain().CurrentBlock()
...
func TestNewPayloadOnInvalidTerminalBlock(t *testing.T) {
	...
	empty, err := api.mbl.Miner().GetSealingBlockSync(parent.Hash(), params.Timestamp, params.SuggestedFeeRecipient, params.Random, true)
	if err != nil {
		t.Fatalf("error preparing payload, err=%v", err)
	}
