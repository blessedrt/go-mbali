...
var (
	...
	// errSnapWithoutmbl is returned if a peer attempts to connect only on the
	// snap protocol without advertizing the mbl main protocol.
	errSnapWithoutmbl = errors.New("peer connected on snap without compatible mbl support")
)

// peerSet represents the collection of active peers currently participating in
// the `mbl` protocol, with or without the `snap` extension.
type peerSet struct {
	peers     map[string]*mblPeer // Peers connected on the `mbl` protocol
	snapPeers int                 // Number of `snap` compatible peers for connection prioritization

	snapWait map[string]chan *snap.Peer // Peers connected on `mbl` waiting for their snap extension
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `mbl`
...
func newPeerSet() *peerSet {
	return &peerSet{
		peers:    make(map[string]*mblPeer),
		snapWait: make(map[string]chan *snap.Peer),
		snapPend: make(map[string]*snap.Peer),
	}
}

// registerSnapExtension unblocks an already connected `mbl` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `mbl` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	// Reject the peer if it advertises `snap` without `mbl` as `snap` is only a
	// satellite protocol meaningful with the chain selection of `mbl`
	if !peer.RunningCap(mbl.ProtocolName, mbl.ProtocolVersions) {
		return errSnapWithoutmbl
	}
	...
	// Inject the peer into an `mbl` counterpart is available, otherwise save for later
...
// by the peerset.
func (ps *peerSet) waitSnapExtension(peer *mbl.Peer) (*snap.Peer, error) {
...
// registerPeer injects a new `mbl` peer into the working set, or returns an error
// if the peer is already known.
func (ps *peerSet) registerPeer(peer *mbl.Peer, ext *snap.Peer) error {
...
func (ps *peerSet) peer(id string) *mblPeer {
	ps.lock.RLock()
...
func (ps *peerSet) peersWithoutBlock(hash common.Hash) []*mblPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	list := make([]*mblPeer, 0, len(ps.peers))
...
func (ps *peerSet) peersWithoutTransaction(hash common.Hash) []*mblPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
...
// len returns if the current number of `mbl` peers in the set. Since the `snap`
// peers are tied to the existence of an `mbl` connection, that will always be a
// subset of `mbl`.
...
func (ps *peerSet) peerWithHighestTD() *mbl.Peer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
