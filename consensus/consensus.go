...
// Package consensus implements different mbali consensus engines.
package consensus
...
type ChainHeaderReader interface {
	...
	gombleader(hash common.Hash, number uint64) *types.Header

	// gombleaderByNumber retrieves a block header from the database by number.
	gombleaderByNumber(number uint64) *types.Header

	// gombleaderByHash retrieves a block header from the database by its hash.
	gombleaderByHash(hash common.Hash) *types.Header
...
type Engine interface {
	// Author retrieves the mbali address of the account that minted the given
