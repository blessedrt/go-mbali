...
import (
	...
	"github.com/mbali/go-mbali/accounts/usbwallet/trezor"
...
// mbali address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.mbaliAddress)
	if _, err := w.trezorExchange(&trezor.mbaliGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.mbaliTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.mbaliTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
