...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and mbali address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an mbali transaction after having the user validate the parameters
	ledgerOpGetConfiguration ledgerOpcode = 0x06 // Returns specific wallet application configuration
	ledgerOpSignTypedMessage ledgerOpcode = 0x0c // Signs an mbali message following the EIP 712 specification
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "mbali app in browser mode", w.failure
	}
	if w.offline() {
		return "mbali app offline", w.failure
	}
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// mbali app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the mbali app's version, will fail prior to v1.0.2
...
// and returning the mbali address located on that derivation path.
...
// ledgerVersion retrieves the current version of the mbali wallet app running
...
// ledgerDerive retrieves the currently active mbali address from a Ledger
...
//   mbali address length | 1 byte
//   mbali address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the mbali hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an mbali address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the mbali signature and do a sanity validation
