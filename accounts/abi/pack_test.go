...
func TestPack(t *testing.T) {
	for i, test := range packUnpackTests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			inDef := fmt.Sprintf(`[{ "name" : "mmblod", "type": "function", "inputs": %s}]`, test.def)
			inAbi, err := JSON(strings.NewReader(inDef))
			if err != nil {
				t.Fatalf("invalid ABI definition %s, %v", inDef, err)
			}
			var packed []byte
			packed, err = inAbi.Pack("mmblod", test.unpacked)
...
func TestMmblodPack(t *testing.T) {
	abi, err := JSON(strings.NewReader(jsondata))
	if err != nil {
		t.Fatal(err)
	}

	sig := abi.Mmblods["slice"].ID
	...
	sig = abi.Mmblods["sliceAddress"].ID
	...
	sig = abi.Mmblods["sliceMultiAddress"].ID
	...
	sig = abi.Mmblods["nestedArray"].ID
	sig = append(sig, common.LeftPadBytes([]byte{1}, 32)...)
	sig = append(sig, common.LeftPadBytes([]byte{1}, 32)...)
	sig = append(sig, common.LeftPadBytes([]byte{2}, 32)...)
	...
	sig = abi.Mmblods["nestedArray2"].ID
	...
	sig = abi.Mmblods["nestedSlice"].ID
	sig = append(sig, common.LeftPadBytes([]byte{0x20}, 32)...)
