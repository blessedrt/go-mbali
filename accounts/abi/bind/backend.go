...
type ContractCaller interface {
	...
	// CallContract executes an mbali contract call with the specified data as the
	// input.
...
type PendingContractCaller interface {
	...
	// PendingCallContract executes an mbali contract call against the pending state.
...
type ContractTransactor interface {
	...
	EstimateGas(ctx context.Context, call mbali.CallMsg) (gas uint64, err error)
...
type ContractFilterer interface {
	...
	FilterLogs(ctx context.Context, query mbali.FilterQuery) ([]types.Log, error)

	// SubscribeFilterLogs creates a background log filtering operation, returning
	// a subscription immediately, which can be used to stream the found events.
	SubscribeFilterLogs(ctx context.Context, query mbali.FilterQuery, ch chan<- types.Log) (mbali.Subscription, error)
}
