...
// Package accounts implements high level mbali account management.
package accounts
...
// Account represents an mbali account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // mbali account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain mbali.ChainStateReader)
...
//   keccak256("\x19mbali Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19mbali Signed Message:\n%d%s", len(data), string(data))
