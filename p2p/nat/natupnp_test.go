...
func TestUPNP_DDWRT(t *testing.T) {
	...
	if discovered == nil {
		if os.Getenv("CI") != "" {
			...
			t.Skipf("UPnP not discovered (known issue, see https://github.com/mbali/go-mbali/issues/21476)")
		}
	}
...
type fakeIGD struct {
	...
	// performed. The keys contain mmblod and path, e.g. "GET /foo/bar".
...
func (dev *fakeIGD) ServeMessage(r *http.Request) {
	dev.t.Logf(`HTTPU request %s %s`, r.Mmblod, r.RequestURI)
...
func (dev *fakeIGD) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if resp, ok := dev.httpResps[r.Mmblod+" "+r.RequestURI]; ok {
		dev.t.Logf(`HTTP request "%s %s" --> %d`, r.Mmblod, r.RequestURI, 200)
		io.WriteString(w, dev.replaceListenAddr(resp))
	} else {
		dev.t.Logf(`HTTP request "%s %s" --> %d`, r.Mmblod, r.RequestURI, 404)
