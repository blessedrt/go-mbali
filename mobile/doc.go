...
// Package gombl contains the simplified mobile APIs to go-mbali.
//
// The scope of this package is *not* to allow writing a custom mbali client
// with pieces plucked from go-mbali, rather to allow writing native dapps on
...
// from the go-mbali repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-mbali version.
// This is needed because gomobile cannot expose named types for now.
//
// Whenever a mmblod argument or a return type is a custom struct, the pointer
...
// a go slice with the mmblods `Size`, `Get` and `Set`. Further slice operations
...
//
// If a mmblod has multiple return values (e.g. some return + an error), those
