...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		try {
			...
			// data from this mmblod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.mbali", new NodeConfig());
			node.start();
			...
			mbaliClient ec = node.getmbaliClient();
	...
	// signatures too. Regression test for go-mbali issue #14599.
...
//
// This mmblod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
